...
import ChatThread from "@/app/chat/[id]/ChatThread";

export const dynamic = "force-dynamic";

export default async function SavedConversationPage({ params }: { params: { id: string } }) {
  const conversation = await prisma.conversation.findUnique({
    where: { id: params.id },
    include: { messages: { orderBy: { createdAt: "asc" }, include: { citations: { include: { corpusPassage: true } } } } },
  });

  if (!conversation) {
    return <div className="card"><p>Conversation not found.</p></div>;
  }
